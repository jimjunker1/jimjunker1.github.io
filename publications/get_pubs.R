...
full_authors = pubs %>% dplyr::select(pubid) %>% purrr::map(~quiet(scholar::get_complete_authors(id = "5pxjNGYAAAAJ", pubid = .x, delay = 0.4 )))
...
# write the bib file
# debugonce(WriteBib)
RefManageR::WriteBib(jrj.bib_flat, file = "./publications/pubs.bib")